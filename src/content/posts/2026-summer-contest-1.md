...
为了方便理解，我们可以把白色看作 `0`，黑色看作 `1`。

#### Step.1 朴素的动态规划

最容易想到的方法是使用 $O(n^3)$ 的动态规划（DP）。

我们定义状态 $f_{i,j}$：表示处理完前 $i$ 行，并且第 $i$ 行的前 $j$ 个格子都是 `0`，从第 $j+1$ 到第 $n$ 个格子都是 `1` 时的最小代价。

它的状态转移方程可以写为：
$$f_{i,j} = \min\{f_{i-1,k} + cost(i,j)\}, \quad k \in [j,n]$$

这里的 $cost(i,j)$ 是一个固定值，表示把第 $i$ 行强制变成“前 $j$ 个为 `0`，后面全为 `1`”所需要的修改代价。

#### Step.2 后缀最小值优化

仔细观察上面的转移方程，我们会发现 $cost(i,j)$ 只和当前的行 $i$ 以及分界点 $j$ 有关，它并不依赖于上一行的状态 $k$。

因此，我们可以把 $cost(i,j)$ 提取到 $\min$ 的外面，将方程简化为：
$$f_{i,j} = cost(i,j) + \min\{f_{i-1,k}\}, \quad k \in [j,n]$$

这样一来，我们只需要在计算完每一行的所有状态后，顺便求一下后缀最小值，就能在 $O(1)$ 的时间内完成状态转移。

#### Step.3 前缀和求代价

至于如何快速求出 $cost(i,j)$，我们可以借助前缀和。

提前预处理出每一行中前 $j$ 个位置里 `1` 的个数。根据定义，把前 $j$ 个位置变成 `0` 需要修改掉这些 `1`，把后面的位置变成 `1` 需要修改掉原有的 `0`。利用前缀和，我们就能在 $O(1)$ 的时间内算出任意 $(i,j)$ 对应的修改代价。
...
本题不要求最优解，且 $N$ 最大仅为 30，基本是随便做了，下面给出一种构造方案：

#### 核心思路

从右向左依次处理每一个位置。在处理第 $i$ 个位置时，假设它右侧的所有位置已经调整完毕，且当前前 $i$ 个位置的状态是完全一致的（全为 `0` 或全为 `1`）。

#### 操作规则

1. 若目标状态 $S_i = 1$：
   - 若前 $i$ 个位置当前全为 `0`，按下 $i$ 次按钮 A，使其全部变为 `1`。
   - 若前 $i$ 个位置当前已全为 `1`，无需操作。
2. 若目标状态 $S_i = 0$：
   - 若前 $i$ 个位置当前全为 `1`，按下 $i$ 次按钮 B，使其全部变为 `0`。
   - 若前 $i$ 个位置当前已全为 `0`，无需操作。

#### 举例

假设目标状态为 `1 0 1`（即 $S_1=1, S_2=0, S_3=1$），初始状态为 `0 0 0`。

1. 处理第 3 位：前 3 位全为 `0`，按下 3 次按钮 A。状态变为 `1 1 1`。
2. 处理第 2 位：前 2 位全为 `1`，按下 2 次按钮 B。状态变为 `0 0 1`。
3. 处理第 1 位：前 1 位全为 `0`，按下 1 次按钮 A。状态变为 `1 0 1`。
...
为了直观地比较 `A` 和 `B` 的数量，我们可以进行赋值抽象：令 `A = 1`，`B = -1`，`C = 0`。

这样一来，“子串中 `A` 的数量严格大于 `B` 的数量”就等价于“该子串对应的数值区间和严格大于 0”。

```text
例如：字符串 ABA 可以抽象为序列 [1, -1, 1]
满足条件的子串有 (1,1)、(3,3)、(1,3)，它们的区间和分别为 1、1、1，均大于 0。
```

#### Step.2 前缀和

区间和可以通过前缀和来快速表示。设 $c_i$ 为前 $i$ 个字符的抽象数值之和。

对于任意区间 $(l, r)$，其区间和可以表示为 $sum = c_r - c_{l-1}$。

要使区间和大于 0，即满足：$c_r - c_{l-1} > 0$，移项可得：$c_r > c_{l-1}$。

#### Step.3 树状数组优化

根据上述推导，原问题转化为了一个经典的统计问题：

当我们固定左端点 $l$ 时，需要统计在右端点 $r \in [l, n]$ 的范围内，有多少个前缀和 $c_r$ 严格大于 $c_{l-1}$。

为了高效完成这一统计，我们可以使用**树状数组**进行求解。
...
$N \leq 3000$。本题为多测，有 $\sum{n^2} \leq 9 \times 10^6$。
...
这类“无论……都能保证……”的问题，本质上是在**最坏的情况**下依然能符合条件。

对于本题而言，最坏的情况包含两层含义：

1. 没选的杯子全是酒。
2. 选中的这些杯子中，只有容量最小的那几杯是酒。

#### Step.2 贪心策略

根据上述最坏情况，最优的贪心策略为：选择容量最大的若干个杯子。

为什么？假设存在两个方案，它们只有一个杯子有区别：

```text
方案 1：a (b) (c) (d)....
方案 2：(a) b (d) (d)....
(x) 代表选了某个杯子，其中 a < b < c < d。 
```

那么在假设最坏的情况下，有酒的杯子为 `a,b,c`：

```text
方案 1：[b] [c]
方案 2：[a] [c]
[x] 代表最终喝下的酒，明显方案 1 优于方案 2
```

我们可以简单理解为：酒总是会在我们选择的杯子中最少的那几个里面，那么我们就尽可能取大的杯子，使得这个“最少”变的最大。
...
$N,Q \leq 2 \times 10^5, T \leq 10^9$

### Analysis

将所有 $i$ 向  $a_i$ 连边，得到的图是基环树森林。对于树上跳跃 k 次求路径和问题，没有比 $\log n$ 更快的做法。

那么只需要考虑带 $\log$ 的做法，本题用倍增即可。
...
本题有着过于独特的数据范围：K = 1e9，这留给我们的时间复杂度只可能是 $\log K$，由此立刻想到矩阵快速幂。
...
平面上有 $N$ 个不在原点的怪物。Takahashi 站在原点，向某方向发射激光可消灭该射线上的所有怪物（同一直线上的怪物会被同时消灭）。

现有 $Q$ 次询问，每次给定两个怪物 $A_j$ 和 $B_j$：
Takahashi 从面向 $A_j$ 的方向开始，**顺时针**旋转直到面向 $B_j$。求此过程中（含起始和终止方向）共消灭了多少个怪物。若 $A_j$ 与 $B_j$ 同向，则不旋转。

$N,Q \leq 2 \times 10^5$

### Analysis

本题的核心在于如何将所有怪物按照“顺时针方向”进行排序，并处理同一射线上的多个点。

一个直观且不易出错的写法是：将平面按顺时针方向划分为 8 个区域（即 4 条坐标轴和 4 个象限），并分别编号为 1~8。

- 第一关键字：按照所属区域的编号从小到大排序。
- 第二关键字：如果两个点处于同一区域（即在同一象限或同一坐标轴上），则通过比较斜率来确定它们的先后顺序。
...
整个大图由 $K \times K$ 个完全相同的子图块平铺而成,这意味着所有子图块在拓扑结构上是同构的。

因此，我们将任意节点 $(x,y)$ 映射为其在基础块 A 中的相对坐标 $(x \mod N, y \mod N)$。

这样以后，我们就可以直接在原图上求最短路径问题。
...
1. 若起点与终点本身相同，则最短路径为 0。

2. 若起点与终点本身不同，则需要求解环路。
...
$N \leq 1000,M \leq 5 \times 10^4$

### Analysis

其实本做法有点极限。赛后跟 qwq 交流，qwq 也有想过用 bitset，但最后怕超时就没用。

用长度为 n 的 01 串表示当前预算下达到最优解时，第 $i$ 个物品是否出现在方案中。

然后用两个 `bitset` `f`,`g` 分别代表方案的交集和并集。
